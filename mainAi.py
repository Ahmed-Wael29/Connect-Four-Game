...
def draw_board(board):
    for c in range(COLS):
        for r in range(ROWS):
            # Drawing the whole board with blue color
            pygame.draw.rect(screen, BLUE, (c * SQUARESIZE, r * SQUARESIZE + SQUARESIZE, SQUARESIZE, SQUARESIZE))
            if board[r][c] == 0:
                # Drawing the free spaces (holes before putting any piece) with black color
                pygame.draw.circle(screen, BLACK, (
                int(c * SQUARESIZE + SQUARESIZE / 2), int(r * SQUARESIZE + SQUARESIZE + SQUARESIZE / 2)), circle_radius)
            elif board[r][c] == 1:
                # Drawing the first piece (user) red color
                pygame.draw.circle(screen, RED, (
                int(c * SQUARESIZE + SQUARESIZE / 2), int(r * SQUARESIZE + SQUARESIZE + SQUARESIZE / 2)), circle_radius)
            else:
                # Drawing the second piece (AI) yellow color
    ...
    if depth == 0 or is_terminal:
        if is_terminal:  # winning move
            if winning_move(board, AI_PIECE):
                return (None, 10000000)  # positive infinity
            elif winning_move(board, PLAYER_PIECE):
                return (None, -10000000)  # negative infinity
            else:
                return (None, 0)
...
# various state tracker variables that use the above functions
# -------------------------------
